            ...
            <div class="col-12">
              ...
              <div class="card shadow mb-4 ">
                ...
                <div class="card-body"> 
                    ...
                    <form action="<?=base_url('admin/control_admin_news/').$action;?>" method="post" enctype="multipart/form-data">
                      <div class="form-group row">
                        <label for="news_id" class="col-sm-2 col-form-label">รหัส<?=$title;?></label>
                        <div class="col-sm-10">
                          <input type="text" readonly  class="form-control" id="news_id" name="news_id" value="<?=$action == 'insert_news' ? $news : $news[0]->news_id;?>" required>
                        </div>
                      </div>
                      <div class="form-group row">
                        <label for="news_topic" class="col-sm-2 col-form-label">หัวเรื่อง</label>
                        <div class="col-sm-10">
                          <input type="text" class="form-control" id="news_topic" name="news_topic" value="<?=$action == 'insert_news' ? '' : $news[0]->news_topic;?>" required>
                        </div>
                      </div>
                      <div class="form-group row">
                        <label for="news_content" class="col-sm-2 col-form-label">เนื้อหา</label>
                        <div class="col-sm-10">
                          <textarea class="form-control" name="news_content" id="news_content" > <?=$action == 'insert_news' ? '' : $news[0]->news_content;?></textarea>
                        </div>
                      </div>
                      <div class="form-group row">
                        <label for="news_date" class="col-sm-2 col-form-label">วันที่ประกาศ</label>
                        <div class="col-sm-2">                       
                          <input type="date" class="form-control" id="news_date" name="news_date" value="<?=$action == 'insert_news' ? date('Y-m-d') : date("Y-m-d", strtotime($news[0]->news_date));?>" >
                        </div>
                      </div>
                      <div class="form-group row">
                      ...
                      <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <div class="upload-btn-wrapper">
                              ...
                              <small id="emailHelp" class="form-text text-muted">  (กรณีไม่มีรูปภาพไม่ต้องใส่)</small>
                            </div>
                        </div> 
                      </div>
                      <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                           ...
                           <img id="blah" class="img-fluid" src="<?php echo  @$news[0]->news_img == '' ? '#' : base_url().'uploads/news/'.$news[0]->news_img; ?>" alt="" /><hr>
                        </div> 
                      </div>
